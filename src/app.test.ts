...
import type { AuthApiOptions } from "./api.ts";
import { choice, sequence } from "./choreography.ts";
import EmailAuthComponent from "./components/email.ts";
import PasswordAuthComponent from "./components/password.ts";
import { AuthStorage } from "./storage.ts";
import type { AuthKvProvider } from "./provider.ts";
...
// through AuthApi directly. Where api.test.ts asserts on a rejected AuthError, the equivalent here is
// a 500 carrying `{ error: <code> }`; where it reads the identity back out of storage, so does this.
describe("App", () => {
  let storage: AuthStorage;
  let apiOptions: AuthApiOptions;
  let post: Post;
  let channelEmail: MemoryAuthChannel;
  let channelEmail2: MemoryAuthChannel;
  let channelSms: MemoryAuthChannel;
  let email: EmailAuthComponent;
  let email2: EmailAuthComponent;
  let password: PasswordAuthComponent;

  beforeEach(() => {
    channelEmail = new MemoryAuthChannel("email");
    channelEmail2 = new MemoryAuthChannel("email2");
    channelSms = new MemoryAuthChannel("phone");
    email = new EmailAuthComponent("email");
    email2 = new EmailAuthComponent("email2");
  ...
  it("should list the components enrolled on the identity", async () => {
    ...
    assertEquals(status, 200);
    // EmailAuthComponent contributes both the identification and the channel it is delivered over, so the
  ...
  it("should subscribe", async () => {
    ...
    assert(
      identity?.components.some((c) =>
        c.kind === "channel" && c.channel === "sms" && c.confirmed
      ),
    );
  });
  ...
  it("should enroll", async () => {
    ...
    assert(
      !identity.components.find((c) =>
        c.kind === "challenge" && c.component === "email2"
      ),
    );
  });
  ...
  it("should unenroll", async () => {
    ...
    assert(
      !identity.components.find((c) =>
        c.kind !== "channel" && c.component === "email2"
      ),
    );
  });
  ...
  it("should not unenroll a component the choreography cannot do without", async () => {
    ...
    assert(
      identity?.components.find((c) =>
        c.kind === "challenge" && c.component === "password"
      ),
    );
  });
  ...
  it("should unenroll a component a choice makes optional", async () => {
    ...
    assert(
      !identity.components.find((c) =>
        c.kind !== "channel" && c.component === "password"
      ),
    );
  });
  ...
  it("should recover password", async () => {
    ...
    const passwordComponent1 = identity1.components.find((c) =>
      c.kind === "challenge" && c.component === "password"
    );
    const passwordComponent2 = identity2?.components.find((c) =>
      c.kind === "challenge" && c.component === "password"
    );
  ...
  it("should surface an unexpected failure as UNKNOWN, not as a business error", async () => {
    ...
    assert(result2.state);
    // OtpAuthComponent.sendPrompt writes the code to KV and does not catch — unlike EmailAuthComponent,
    ...
    // AuthError keeps `code` as its only own-enumerable property, so the body carries nothing else.
    assertEquals(Object.keys(thrown), ["error"]);
  });

  // No counterpart in api.test.ts: these cover what the edge itself adds on top of AuthApi — reading the
  // access token off the Authorization header, the caller off the connection, and body validation.
  describe("http", () => {
    ...
    // AuthResponseState carries a Date; what reaches the client is the ISO string c.json produces, which
