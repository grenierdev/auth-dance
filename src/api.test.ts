...
describe("Api", () => {
  ...
  it("should give each flow its own state duration", async () => {
    const perFlowApi = new AuthApi({
      ...apiOptions,
      advanced: { sign_in_duration: 30, recover_duration: 15 * 60 },
    });
    const secondsFromNow = (expireAt: Date) =>
  ...
  it("should reject an access token carrying no sign-in date", async () => {
    ...
    const rejected = await assertRejects(() => api.signOut(forged), AuthError);
  ...
  describe("rate limit", () => {
    ...
    function limitedApi(advanced: AuthApiOptions["advanced"]): AuthApi {
      return new AuthApi({ ...apiOptions, advanced });
    }
